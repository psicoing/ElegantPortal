...
export function InvestorsDialog({ children }: InvestorsDialogProps) {
  ...
  // Al hacer clic en el activador (children), abrir el modal
  const handleOpen = (e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    setIsOpen(true);
    // Bloquear el desplazamiento cuando se abre
    document.body.style.overflow = 'hidden';
  };

  // Al hacer clic en cerrar, cerrar el modal
  const handleClose = () => {
    setIsOpen(false);
    // Restaurar el desplazamiento cuando se cierra
    document.body.style.overflow = 'auto';
    // Resetear la pestaña activa después de cerrar
  ...
  // Limpiar el estilo de overflow en caso de desmontar
  useEffect(() => {
    return () => {
      document.body.style.overflow = 'auto';
    };
